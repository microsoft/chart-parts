...
export function createDomain({
	...
	return buildMark(MarkType.Rule)
		...
		.items({
			...
			[rangeStartProperty]: minRange,
			[rangeEndProperty]: maxRange + strokeWidth(axis),
...
function crossValue(axis: Axis, thickness: number): number {
	...
	switch (orient) {
		case AxisOrientation.Bottom: {
			result = (domainWidth || DEFAULT_DOMAIN_WIDTH) / 2
			break
		}
		case AxisOrientation.Left: {
			result = thickness - (domainWidth || DEFAULT_DOMAIN_WIDTH) / 2
			break
		}
